...
public class BmsDAOImpl_old implements BmsDAO{

	DataSource dataSource;
	
	private static BmsDAOImpl_old instance = new BmsDAOImpl_old();
	...
	public int search(int i, ArrayList<String> list, ArrayList<BookDTO> bdtos, ArrayList<GuestDTO> gdtos, ArrayList<SalesInfoDTO> sdtos, ArrayList<BoardDTO> bodtos) {
		...
		try{
			...
			//cart 책 담기
				...
				//sql = "SELECT b.title as title ,s.isbn as isbn from salesinfo s join book1 b on s.isbn = b.isbn where guestid = ?";
				...
				sql = "Select * "
					...
					//sql = "Select sum(closing) From salesInfo";
					...
					//sql = "SELECT distinct b.title, distinct s.isbn, s.salesnum, distinct b.price, distinct s.salesday, distinct s.state from salesinfo s join book1 b on s.isbn = b.isbn where guestid = ?";
					...
					if(list.size() < 4){
						...
						sql = "Select s.salesnum as salesnum, s.isbn as isbn, b.title as title, b.price as price, s.salesday as salesday, s.state as state, s.salesstock as salesstock, s.guestid as guestid  From salesinfo s join book1 b on s.isbn = b.isbn Where " + name + " = ? order by s.salesNum desc";
						pstmt = conn.prepareStatement(sql);
						pstmt.setString(1, list.get(2));
						System.out.println("sql : " + sql);
					...
					sql = "Select count(*) From salesInfo Where " + name + " = ?";	
					pstmt = conn.prepareStatement(sql);
